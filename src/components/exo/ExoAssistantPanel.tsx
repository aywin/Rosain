...
interface ExoAssistantPanelProps {
    ...
    // Prop optionnelle pour recevoir une capture depuis la page PDF
...
export default function ExoAssistantPanel({
    ...
    // Expose handleImageUpload au parent (page PDF) via la prop onImageCapture
    useEffect(() => {
        if (onImageCapture) {
            onImageCapture(handleImageUpload);
        }
    }, [onImageCapture, activeExercises]);
    ...
    // Drag & drop (images uniquement, plus de PDF)
    useEffect(() => {
            ...
            {isDragging && (
                <div className="absolute inset-0 bg-green-500/10 border-4 border-dashed border-green-500 z-50 flex items-center justify-center">
                    <div className="text-center bg-white p-6 rounded-xl shadow-lg">
                        ...
                        <p className="text-lg font-semibold text-green-700">Dépose ton image ici</p>
                    </div>
                </div>
            )}
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b bg-gradient-to-r from-green-50 to-green-100">
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        <button
                            onClick={onClose}
                            className="ml-auto p-1.5 hover:bg-green-200 rounded transition"
                        >
                            <X size={18} />
                        </button>
                    </div>
                    ...
                    {activeExercises.length > 0 && (
                        <div className="flex items-center gap-2 flex-wrap text-xs">
                            {activeExercises.slice(0, 3).map((ex) => (
                                <div
                                    key={ex.id}
                                    className="flex items-center gap-1 bg-white px-2 py-1 rounded-full border border-green-200"
                                >
                        ...
                        <div className="max-w-md">
                            <div className="mb-6">
                                ...
                                <p className="text-sm text-gray-600">
                                    Prends en photo ton exercice ou sélectionne une zone dans le PDF
                                </p>
                            </div>
                            ...
                            <div className="mt-6 text-xs text-gray-500 space-y-1">
                                ...
                                <p>• Sélectionner une zone dans le PDF à gauche</p>
                            </div>
                        </div>
                    </div>
                ) : (
            ...
            <div className="border-t bg-white p-4">
                <div className="flex items-end gap-2">
                    ...
                    <textarea
                        ...
                        placeholder={
                            activeExercises.length > 0
                                ? "Pose ta question sur les exercices..."
                                : "Pose ta question ou sélectionne une zone dans le PDF..."
                        }
                        value={question}
                        onChange={(e) => {
                ...
                <p className="text-xs text-gray-500 mt-2 text-center">
                    <kbd className="px-1.5 py-0.5 bg-gray-200 rounded text-xs">Entrée</kbd> pour
                    envoyer •{" "}
                    <kbd className="px-1.5 py-0.5 bg-gray-200 rounded text-xs">Ctrl+V</kbd> pour
                    coller une image
                </p>
            </div>
        </div>
